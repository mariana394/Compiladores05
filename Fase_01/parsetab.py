...
_lr_productions = [
  ...
  ('return -> RETURN exp SEMICOLON','return',3,'p_return','parser.py',167),
  ('return -> empty','return',1,'p_return','parser.py',168),
  ('program_main -> MAIN LBRACKET program_vars inner_body RBRACKET','program_main',5,'p_program_main','parser.py',172),
  ('body -> LBRACKET inner_body RBRACKET','body',3,'p_body','parser.py',177),
  ('inner_body -> statement inner_body','inner_body',2,'p_inner_body','parser.py',181),
  ('inner_body -> empty','inner_body',1,'p_inner_body','parser.py',182),
  ('assign -> variable ASSIGN specialf_assign SEMICOLON','assign',4,'p_assign','parser.py',187),
  ('specialf_assign -> exp','specialf_assign',1,'p_specialf_assign','parser.py',190),
  ('specialf_assign -> special_function','specialf_assign',1,'p_specialf_assign','parser.py',191),
  ('specialf_assign -> read','specialf_assign',1,'p_specialf_assign','parser.py',192),
  ('condition -> IF LPAREN exp RPAREN body condition2 SEMICOLON','condition',7,'p_condition','parser.py',196),
  ('condition2 -> ELSE body','condition2',2,'p_condition2','parser.py',199),
  ('condition2 -> empty','condition2',1,'p_condition2','parser.py',200),
  ('print -> PRINT LPAREN print_many RPAREN SEMICOLON','print',5,'p_print','parser.py',204),
  ('print_type -> exp','print_type',1,'p_print_type','parser.py',208),
  ('print_many -> print_type print_many2','print_many',2,'p_print_many','parser.py',211),
  ('print_many2 -> COMMA print_type print_many2','print_many2',3,'p_print_many2','parser.py',214),
  ('print_many2 -> empty','print_many2',1,'p_print_many2','parser.py',215),
  ('read -> READ LPAREN variable RPAREN','read',4,'p_read','parser.py',221),
  ('cycle -> for','cycle',1,'p_cycle','parser.py',225),
  ('cycle -> while','cycle',1,'p_cycle','parser.py',226),
  ('while -> DO body WHILE LPAREN exp RPAREN SEMICOLON','while',7,'p_while','parser.py',230),
  ('for -> FOR LPAREN ID TO for_end RPAREN body SEMICOLON','for',8,'p_for','parser.py',234),
  ('for_end -> CTE_INT','for_end',1,'p_for_end','parser.py',237),
  ('for_end -> ID','for_end',1,'p_for_end','parser.py',238),
  ('call_function -> ID LPAREN exp exp_many RPAREN','call_function',5,'p_call_function','parser.py',242),
  ('exp_many -> COMMA exp exp_many','exp_many',3,'p_exp_many','parser.py',247),
  ('exp_many -> empty','exp_many',1,'p_exp_many','parser.py',248),
  ('statement -> assign','statement',1,'p_statement','parser.py',252),
  ('statement -> condition','statement',1,'p_statement','parser.py',253),
  ('statement -> print','statement',1,'p_statement','parser.py',254),
  ('statement -> cycle','statement',1,'p_statement','parser.py',255),
  ('statement -> call_function','statement',1,'p_statement','parser.py',256),
  ('special_function -> exploration','special_function',1,'p_special_function','parser.py',260),
  ('special_function -> financial_state','special_function',1,'p_special_function','parser.py',261),
  ('special_function -> dummi_regression','special_function',1,'p_special_function','parser.py',262),
  ('special_function -> season_analysis','special_function',1,'p_special_function','parser.py',263),
  ('special_function -> trend_prediction','special_function',1,'p_special_function','parser.py',264),
  ('special_function -> model_predict','special_function',1,'p_special_function','parser.py',265),
  ('exploration -> EXPLORATION LPAREN variable explore_var explor_cte RPAREN','exploration',6,'p_exploration','parser.py',271),
  ('explore_var -> COMMA variable','explore_var',2,'p_explore_var','parser.py',274),
  ('explore_var -> empty','explore_var',1,'p_explore_var','parser.py',275),
  ('explor_cte -> COMMA CTE_INT','explor_cte',2,'p_explor_cte','parser.py',278),
  ('explor_cte -> empty','explor_cte',1,'p_explor_cte','parser.py',279),
  ('financial_state -> FINANCIAL_STATE LPAREN variable COMMA variable COMMA variable COMMA variable RPAREN','financial_state',10,'p_financial_state','parser.py',283),
  ('season_analysis -> SEASON_ANALYSIS LPAREN variable RPAREN','season_analysis',4,'p_season_analysis','parser.py',287),
  ('trend_prediction -> TREND_PREDICTION LPAREN variable COMMA CTE_INT COMMA CTE_INT COMMA CTE_INT RPAREN','trend_prediction',10,'p_trend_prediction','parser.py',293),
  ('dummi_regression -> DUMMI_REGRESSION LPAREN variable COMMA variable dr_array dr_int RPAREN','dummi_regression',8,'p_dummi_regression','parser.py',297),
  ('dr_array -> COMMA LSQBRACKET CTE_CHAR dr_array_mp RSQBRACKET','dr_array',5,'p_dr_array','parser.py',300),
  ('dr_array -> empty','dr_array',1,'p_dr_array','parser.py',301),
  ('dr_array_mp -> COMMA CTE_CHAR dr_array_mp','dr_array_mp',3,'p_dr_array_mp','parser.py',304),
  ('dr_array_mp -> empty','dr_array_mp',1,'p_dr_array_mp','parser.py',305),
  ('dr_int -> COMMA CTE_INT','dr_int',2,'p_dr_int','parser.py',308),
  ('dr_int -> empty','dr_int',1,'p_dr_int','parser.py',309),
  ('model_predict -> MODEL_PREDICT LPAREN variable COMMA variable COMMA RPAREN','model_predict',7,'p_model_predict','parser.py',313),
  ('exp -> t_exp exp_or','exp',2,'p_exp','parser.py',318),
  ('exp_or -> OR exp','exp_or',2,'p_exp_or','parser.py',321),
  ('exp_or -> empty','exp_or',1,'p_exp_or','parser.py',322),
  ('t_exp -> expression t_exp_and','t_exp',2,'p_t_exp','parser.py',326),
  ('t_exp_and -> AND t_exp','t_exp_and',2,'p_t_exp_and','parser.py',329),
  ('t_exp_and -> empty','t_exp_and',1,'p_t_exp_and','parser.py',330),
  ('expression -> m_exp expression_comp','expression',2,'p_expression','parser.py',334),
  ('expression_comp -> expression_comp_2 m_exp','expression_comp',2,'p_expression_comp','parser.py',337),
  ('expression_comp -> empty','expression_comp',1,'p_expression_comp','parser.py',338),
  ('expression_comp_2 -> EQUAL','expression_comp_2',1,'p_expression_comp_2','parser.py',341),
  ('expression_comp_2 -> NOTEQUAL','expression_comp_2',1,'p_expression_comp_2','parser.py',342),
  ('expression_comp_2 -> LTHAN','expression_comp_2',1,'p_expression_comp_2','parser.py',343),
  ('expression_comp_2 -> GTHAN','expression_comp_2',1,'p_expression_comp_2','parser.py',344),
  ('m_exp -> term m_exp_sr','m_exp',2,'p_m_exp','parser.py',349),
  ('m_exp_sr -> m_exp_sr_2 m_exp','m_exp_sr',2,'p_m_exp_sr','parser.py',352),
  ('m_exp_sr -> empty','m_exp_sr',1,'p_m_exp_sr','parser.py',353),
  ('m_exp_sr_2 -> PLUS','m_exp_sr_2',1,'p_m_exp_sr_2','parser.py',356),
  ('m_exp_sr_2 -> MINUS','m_exp_sr_2',1,'p_m_exp_sr_2','parser.py',357),
  ('term -> sub_factor term_pc','term',2,'p_term','parser.py',361),
  ('term_pc -> term_pc_2 term','term_pc',2,'p_term_pc','parser.py',364),
  ('term_pc -> empty','term_pc',1,'p_term_pc','parser.py',365),
  ('term_pc_2 -> MULTIPLY','term_pc_2',1,'p_term_pc_2','parser.py',368),
  ('term_pc_2 -> DIVIDE','term_pc_2',1,'p_term_pc_2','parser.py',369),
  ('term_pc_2 -> MODULE','term_pc_2',1,'p_term_pc_2','parser.py',370),
  ('sub_factor -> factor sub_factor_pc','sub_factor',2,'p_sub_factor','parser.py',374),
  ('sub_factor_pc -> MODULE sub_factor','sub_factor_pc',2,'p_sub_factor_pc','parser.py',377),
  ('sub_factor_pc -> empty','sub_factor_pc',1,'p_sub_factor_pc','parser.py',378),
  ('factor -> factor_exp','factor',1,'p_factor','parser.py',384),
  ('factor -> factor_cte','factor',1,'p_factor','parser.py',385),
  ('factor -> variable','factor',1,'p_factor','parser.py',386),
  ('factor -> call_function','factor',1,'p_factor','parser.py',387),
  ('factor_exp -> LPAREN exp RPAREN','factor_exp',3,'p_factor_exp','parser.py',390),
  ('factor_cte -> CTE_INT','factor_cte',1,'p_factor_cte','parser.py',393),
  ('factor_cte -> CTE_FLOAT','factor_cte',1,'p_factor_cte','parser.py',394),
  ('factor_cte -> CTE_CHAR','factor_cte',1,'p_factor_cte','parser.py',395),
]
